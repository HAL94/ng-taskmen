...
@Component({
  selector: 'chat-form',
  template: `
    <form #chatTextForm="ngForm" (ngSubmit)="onSendMessage(chatTextForm)" class="flex flex-row items-center w-full px-20 relative">
      <div class="bg-transparent flex flex-row px-2 py-2 my-2 justify-end items-center min-w-0 flex-grow border rounded-3xl">
        <div class="w-full">
          ...
          <app-file-preview  *ngIf="filePreview$ | async as filePreview" [filePreview]="filePreview"></app-file-preview>
        </div>

        <div class="self-baseline flex flex-row">
          <app-emoji-picker class="mx-1" (selectedEmoji)="setMessage($event)"></app-emoji-picker>
          <app-file-upload class="mx-1"></app-file-upload>
        </div>
      </div>
      <div class="ml-4">
        <button mat-icon-button aria-label="Send Message" type="submit" [disabled]="chatTextForm.invalid" class="disabled:cursor-not-allowed disabled:opacity-60">
